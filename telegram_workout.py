...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Handle Telegram Mini App web_app_data (workout logger, etc.)
            try:
                from telegram.ext import filters as _f
                if hasattr(_f.StatusUpdate, "WEB_APP_DATA"):
                    self._app.add_handler(TelegramMessageHandler(
                        _f.StatusUpdate.WEB_APP_DATA,
    ...
    async def _handle_callback_query(
        ...
        # --- Workout callbacks (wk:action[:param]) ---
        if data.startswith("wk:"):
            await self._handle_workout_callback(query, data)
            return
    ...
    async def _handle_workout_callback(self, query: Any, data: str) -> None:
        """Handle wk: prefixed inline keyboard callbacks from the workout logger."""
        parts = data.split(":", 2)
        action = parts[1] if len(parts) > 1 else ""

        if action == "log":
            # Re-send the workout button so user can open the Mini App again
            await query.answer()
            try:
                from telegram import InlineKeyboardButton as _IKB, InlineKeyboardMarkup as _IKM, WebAppInfo as _WAI
                keyboard = _IKM([[_IKB("🏋️ Open Workout Logger", web_app=_WAI(url="https://slimchillers.github.io/workout-mini-app/"))]])
                await query.message.reply_text("Tap below to log your workout:", reply_markup=keyboard)
            except Exception as e:
                logger.error("[Telegram] wk:log callback error: %s", e)
            return

        if action == "stats":
            exercise = parts[2] if len(parts) > 2 else ""
            await query.answer(text=f"Loading stats for {exercise}…")
            try:
                history = self._read_log_history(exercise=exercise, limit=200)
                # Keep only the last 5 sessions for this exercise
                sessions = history[-5:]
                if not sessions:
                    text = f"📊 *{exercise}*\nNo history found yet."
                else:
                    lines = [f"📊 *{exercise}* — last {len(sessions)} sessions", "─────────────────────────"]
                    best_score = max((r["weight"] * r["reps"] for r in sessions), default=0)
    ...
    def _read_log_history(self, exercise: str | None = None, limit: int = 200) -> list[dict]:
        """Read workout rows from Google Sheet.  Returns list of dicts with keys:
        date, workout, exercise, weight, reps, sets, notes.
        Sheet column order: A=Date, B=WorkoutPhase, C=Exercise, D=Sets, E=Reps, F=Weight, G=Notes."""
        ...
        sheet_id = "1ai9b0Cls80BQQASKMkFaCqrUsUL0bVnAYKwu08q8i_Y"
        import subprocess
        try:
            result = subprocess.run(
                f'HERMES_HOME=/opt/data/profiles/coding-premium {gapi} sheets get {sheet_id} "Sheet1!A:G"',
        ...
        for row in rows[-limit:]:
            ...
            # Skip header row
            if ex_name.lower() in ("exercise", ""):
                continue
            if exercise and ex_name.lower() != exercise.lower():
                continue
            # Sheet columns: D=Sets(3), E=Reps(4), F=Weight(5)
            try:
                sets_val = int(row[3]) if len(row) > 3 and row[3] else 0
            except (ValueError, TypeError):
                sets_val = 0
            try:
            ...
            out.append({
                ...
                "workout": row[1] if len(row) > 1 else "",
                "exercise": ex_name,
                "sets": sets_val,
                "reps": reps,
                "weight": weight,
    ...
    def _is_new_pr(self, exercise: str, weight: float, reps: int, history: list[dict]) -> bool:
        """Return True if weight×reps is a new all-time record for this exercise."""
        if weight <= 0 or reps <= 0:
            return False
        current_score = weight * reps
        for row in history:
            if row["exercise"].lower() == exercise.lower():
                prev = row["weight"] * row["reps"]
    ...
    async def _handle_web_app_data(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            logger.warning("[Telegram] Invalid web_app_data JSON")
            return
        
        if data.get("type") != "workout_log":
            return
        
        await self._process_workout_log(update, data)

    async def _process_workout_log(self, update: Update, data: dict) -> None:
        """Parse workout log data, append to Google Sheet, and send a rich confirmation."""
        import subprocess
        ...
        rows = []
        logged_exercises = []  # (name, weight, reps, sets)
        ...
        if cardio_type and isinstance(cardio_min, (int, float)) and cardio_min > 0:
            rows.append([date, workout, f"Cardio: {cardio_type}", 1, cardio_min, "", notes])
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error("[Telegram] Google Sheets append failed: %s", result.stderr)
        ...
        top_lift = max(logged_exercises, key=lambda x: x[1] * x[2], default=None)

        # Format date nicely
        try:
            from datetime import datetime as _dt
            day_label = _dt.strptime(date, "%Y-%m-%d").strftime("%a %-d %b")
        except Exception:
            day_label = date

        lines = [f"✅ *{workout}* · {day_label} · {len(logged_exercises)} exercise{'s' if len(logged_exercises) != 1 else ''}"]
        lines.append("━━━━━━━━━━━━━━━━━━━━━━")
        ...
        # Inline buttons: Stats for first exercise + Log Again
        try:
            keyboard_rows = []
            if logged_exercises:
                first_ex = logged_exercises[0][0]
                keyboard_rows.append([
                    InlineKeyboardButton("📊 Stats", callback_data=f"wk:stats:{first_ex}"),
                    InlineKeyboardButton("🔄 Log Again", callback_data="wk:log"),
                ])
            keyboard = InlineKeyboardMarkup(keyboard_rows) if keyboard_rows else None
            await update.message.reply_text(confirmation, parse_mode="Markdown", reply_markup=keyboard)
        except Exception:
